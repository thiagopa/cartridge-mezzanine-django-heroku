...
def process(request, order_form, order):
    ...
    logger.debug("Post no Paypal com : \n%s" % trans)
    try:
        f = urllib2.urlopen(conn)
        all_results = f.read()
    except urllib2.URLError:
        raise CheckoutError(_("Could not talk to PayPal payment gateway"))
    parsed_results = QueryDict(all_results)
    logger.debug("Resposta do Paypal: \n%s" % parsed_results)
    state = parsed_results['ACK']
    if not state in ["Success", "SuccessWithWarning"]:
        raise CheckoutError(_(parsed_results['L_LONGMESSAGE0']))
